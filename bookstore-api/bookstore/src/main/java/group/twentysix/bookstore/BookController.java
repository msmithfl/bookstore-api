...
public class BookController {
    ...
    public ResponseEntity<List<Book>> getBooksByGenres(@PathVariable String genres) {
        return new ResponseEntity<List<Book>>(bookRepository.findByGenres(genres), HttpStatus.OK);
    }

    // FEATURE 2: GET TOP 10 SELLERS
    @GetMapping("/top-sellers")
    public ResponseEntity<List<Book>> getTopSellers() {
        return new ResponseEntity<List<Book>>(bookRepository.findTop10ByOrderByAmountSoldDesc(), HttpStatus.OK);
    }
